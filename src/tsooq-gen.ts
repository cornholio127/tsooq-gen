...
const transaction: (pool: Pool, runnable: (client: PoolClient) => Promise<any>) => Promise<void> = (pool, runnable) => new Promise<void>((resolve, reject) => {
...
const loadSchema: (pool: Pool, schemaName: string) => Promise<string[]> = (pool, schemaName) => new Promise<string[]>((resolve, reject) => {
  pool.query('SELECT table_name FROM information_schema.tables WHERE table_schema=$1 AND table_type=$2', [schemaName, 'BASE TABLE'], (err, result) => {
    if (err) {
      reject(err);
    } else {
...
const loadTableDefinition: (pool: Pool, schemaName: string, tableName: string) => Promise<TableDefinition> = (pool, schemaName, tableName) => new Promise<TableDefinition>((resolve, reject) => {
  pool.query('SELECT ordinal_position, column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema=$1 AND table_name=$2 ORDER BY ordinal_position ASC', [schemaName, tableName], (err, result) => {
    if (err) {
      reject(err);
    } else {
      resolve({
...
const tsModel: (td: TableDefinition) => string = td => {
  ...
  td.fields.forEach(field => {
    s += `  static readonly ${toStaticFieldName(field.fieldName)}: Field<${toTsDataType(field.dataType)}> = new FieldImpl<${toTsDataType(field.dataType)}>(${className}._TABLE_NAME, ${field.ordinal}, '${field.fieldName}', undefined, '${field.dataType}', ${field.isNullable}, ${field.hasDefault});\n`;
  });
...
const generateModel: (pool: Pool, schemaName: string, outputDir: string) => Promise<void> = async (pool, schemaName, outputDir) => {
